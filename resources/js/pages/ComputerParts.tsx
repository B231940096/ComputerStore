...
interface ComputerPartsProps {
  cpu: Product[]
  gpu: Product[]
  ram: Product[]
  storage: Product[]
  motherboard: Product[]
  power_supply: Product[]
  fan: Product[]
  cpu_cooler: Product[]
  case: Product[]
}
const categories = [
  { id: 'cpu', label: 'Процессор', icon: <Cpu className="h-5 w-5" /> },
  { id: 'gpu', label: 'График карт', icon: <Microchip className="h-5 w-5" /> },
  { id: 'ram', label: 'RAM', icon: <MemoryStick className="h-5 w-5" /> },
  { id: 'storage', label: 'Хадгалалт', icon: <HardDrive className="h-5 w-5" /> },
  { id: 'motherboard', label: 'Эх хавтан', icon: <SquareSquare className="h-5 w-5" /> },
  { id: 'power_supply', label: 'Тэжээл', icon: <Power className="h-5 w-5" /> },
  { id: 'fan', label: 'Сэнс', icon: <Fan className="h-5 w-5" /> },
  { id: 'cpu_cooler', label: 'Хөргүүр', icon: <EqualApproximately className="h-5 w-5" /> },
  { id: 'case', label: 'Кэйс', icon: <PcCase className="h-5 w-5" /> },
] as const
...
export default function Computerparts(props: ComputerPartsProps) {
  ...
  const productsByCategory: Record<string, Product[]> = {
    ...
    power_supply: props.power_supply,
    fan: props.fan,
    cpu_cooler: props.cpu_cooler,
    case: props.case,
  }

  return (
    <AppLayout breadcrumbs={breadcrumbs}>
      <Tabs value={active} onValueChange={(value) => setActive(value as CategoryId)} defaultValue="cpu" className="w-full">
...
data-[state=active]:bg-primary data-[state=active]:text-primary-foreground
      ...
      "
        ...
        {Object.entries(productsByCategory).map(([key, items]) => (
          <TabsContent key={key} value={key} className="mt-20">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-4 px-8">
              {productsByCategory[active].map((product, index) => (
                <Card
                  key={index}
